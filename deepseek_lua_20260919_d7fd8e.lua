...
if not ClientData or not InventoryDB or not KindDB then
    error("Modules requis non charges")
end
...
function SpawnPetByName(petName)
    ...
    if item then
        ...
        return nil, "Echec du spawn : " .. petName
    end
end
...
function SpawnToyByName(toyName)
    ...
    if item then
        ...
        return nil, "Echec du spawn : " .. toyName
    end
end
...
function SpawnAllVariants(petName)
    ...
    return count, count .. " variantes spawn"
end
...
function SpawnRandomPets(amount)
    ...
    if #allKinds == 0 then return 0, "Aucun pet trouve" end
    local count = 0
    for i = 1, amount do
    ...
    return count, count .. " pets aleatoires spawn"
end

-- ============================================================
-- Ajout visuel local dans le trade
-- ============================================================
...
local function visualAddPetToMyOffer(petName, flags)
    ...
    return true, "Ajoute visuellement : " .. petName
end
...
makeSectionLabel("Controles du pet", 6)
...
equipInfo.Text = "Aucun pet equipe"
...
local equipBtn = makeCtrlBtn("Equiper", 1)
...
local function updatePetControls(item)
    if item and item.unique and SpawnedPets[item.unique] then
        equipInfo.Text = "Equipe : " .. (item.kind or "Inconnu")
    else
        equipInfo.Text = "Aucun pet equipe"
    end
end
...
equipBtn.MouseButton1Click:Connect(function()
    ...
    if not petData then
        game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = "Pet introuvable.", Duration = 3})
        return
    end
    BPEquip(petData)
    updatePetControls(petData)
    game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = "Equipe " .. (petData.kind or "pet"), Duration = 3})
end)

rideBtn.MouseButton1Click:Connect(function()
    if not EquippedPet then
        game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = "Aucun pet equipe.", Duration = 3})
        return
    end
    ...
    game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = "Ride active.", Duration = 2})
end)
...
flyBtn.MouseButton1Click:Connect(function()
    ...
    game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = "Fly active.", Duration = 2})
end)
...
unequipBtn.MouseButton1Click:Connect(function()
    ...
    game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = "Pet retire.", Duration = 2})
end)
...
renameBtn.MouseButton1Click:Connect(function()
    ...
    if success then
        game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = "Pet renomme en " .. newName, Duration = 3})
        renameBox.Text = ""
    else
        game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = "Echec : " .. tostring(err), Duration = 3})
    end
end)
...
local function equipPetWear(wearName)
    if not EquippedPet then
        game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = "Equipe un pet d'abord.", Duration = 3})
        return
    end
    ...
    task.spawn(function()
        ...
        if not wearEntry then
            game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = "Entree pet wear manquante.", Duration = 3})
        ...
        if not petModel then
            game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = "Modele du pet introuvable.", Duration = 3})
            pcall(setthreadidentity, 8)
            return
        end
        ...
        if not ok or not accessoryAsset then
            game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = "Echec du telechargement.", Duration = 3})
            pcall(setthreadidentity, 8)
            return
        end
        ...
        if success and result then
            game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = wearName .. " equipe sur le pet !", Duration = 4})
        else
            game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = "Echec de l'equipement.", Duration = 3})
        end
    end)
end
