...
-- Equip, Ride, Fly, Rename, Pet Wear, Visual Trade, Themes
-- ============================================================
...
local function EquipPetWear(wearName)
    ...
    task.spawn(function()
        ...
        local ok, err = pcall(function()
            ...
            if not PAEquip or not DL then return end
            local accessoryAsset = DL.promise_download_copy("PetAvatarResources", wearEntry.model_handle):expect()
            if accessoryAsset then
                PAEquip.equip_accessory({
                    pet_model            = petModel,
        ...
        if ok then
            game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = wearName .. " equipped!", Duration = 3})
        else
            game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = "Equip failed.", Duration = 3})
        end
    end)
    return true, "Equipping " .. wearName
end

-- ============================================================
-- Visual Trade Add (client-only, other player can't see)
-- ============================================================
local function VisualAddToTrade(petName, flags)
    local TradeApp = UIManager.apps and UIManager.apps.TradeApp
    if not TradeApp then return false, "TradeApp unavailable." end
    local state
    pcall(function() state = TradeApp:_get_local_trade_state() end)
    if not state then return false, "No trade in progress." end
    local kind = FindPetKind(petName)
    if not kind then return false, "Pet not found: " .. petName end
    local myOffer = (state.sender == Players.LocalPlayer) and state.sender_offer or state.recipient_offer
    if not myOffer then return false, "My offer unavailable." end
    if #myOffer.items >= 18 then return false, "Offer full." end
    ...
    pcall(function()
        if TradeApp._render_message_in_trade_chat then
            TradeApp:_render_message_in_trade_chat(nil, "You added " .. petName .. " (visual only)", false)
        end
    end)
    return true, "Visual add: " .. petName
end

-- Block server sync of _visual_only items
do
    local ok, AddItemRemote = pcall(function()
        return RouterClient.get and RouterClient.get("TradeAPI/AddItem")
    end)
    if ok and AddItemRemote then
    ...
    local function updateDescendants(obj)
        ...
        if obj:IsA("Frame") then
            ...
            elseif obj:IsA("Frame") and obj.Parent == content then
                ...
                elseif obj == petsPanel or obj == toysPanel then
...
-- Pets panel as ScrollingFrame so we can scroll to themes at the bottom
...
petsPad.PaddingTop = UDim.new(0, 4)
petsPad.PaddingBottom = UDim.new(0, 12)
petsPad.PaddingLeft = UDim.new(0, 2)
...
wearBox.Text = ""
wearBox.PlaceholderText = "Pet wear name..."
...
wearBtn.Size = UDim2.new(0.33, 0, 1, 0)
wearBtn.BackgroundColor3 = Color3.fromRGB(45, 45, 65)
wearBtn.Text = "Equip"
wearBtn.Font = Enum.Font.GothamBold
wearBtn.TextSize = 10
wearBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
wearBtn.LayoutOrder = 2
wearBtn.Parent = wearRow
Instance.new("UICorner", wearBtn).CornerRadius = UDim.new(0, 6)
...
weStroke.Parent = wearBtn

wearBtn.MouseButton1Click:Connect(function()
    local name = wearBox.Text
    if name == "" then
        game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = "Enter a pet wear name.", Duration = 3})
        return
    end
    local ok, msg = EquipPetWear(name)
    game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = tostring(msg), Duration = 3})
end)

-- ============================================================
-- Visual Trade Add Section
-- ============================================================
local vtLabel = Instance.new("TextLabel")
vtLabel.Size = UDim2.new(1, 0, 0, 14)
vtLabel.BackgroundTransparency = 1
vtLabel.Text = "Visual Trade Add (client only)"
vtLabel.Font = Enum.Font.GothamBold
vtLabel.TextSize = 11
vtLabel.TextColor3 = Color3.fromRGB(120, 220, 255)
vtLabel.TextXAlignment = Enum.TextXAlignment.Left
vtLabel.LayoutOrder = 9
vtLabel.Parent = petsPanel

local vtRow = Instance.new("Frame")
vtRow.Size = UDim2.new(1, 0, 0, 26)
vtRow.BackgroundTransparency = 1
vtRow.LayoutOrder = 10
vtRow.Parent = petsPanel
local vtRowLayout = Instance.new("UIListLayout")
vtRowLayout.FillDirection = Enum.FillDirection.Horizontal
vtRowLayout.Padding = UDim.new(0, 5)
vtRowLayout.SortOrder = Enum.SortOrder.LayoutOrder
vtRowLayout.Parent = vtRow

local vtBox = Instance.new("TextBox")
vtBox.Size = UDim2.new(0.65, 0, 1, 0)
vtBox.BackgroundColor3 = Color3.fromRGB(35, 35, 50)
vtBox.Text = ""
vtBox.PlaceholderText = "Pet name..."
vtBox.Font = Enum.Font.SourceSans
vtBox.TextSize = 11
vtBox.TextColor3 = Color3.fromRGB(255, 255, 255)
vtBox.PlaceholderColor3 = Color3.fromRGB(120, 120, 160)
vtBox.ClearTextOnFocus = false
vtBox.TextXAlignment = Enum.TextXAlignment.Left
vtBox.LayoutOrder = 1
vtBox.Parent = vtRow
Instance.new("UICorner", vtBox).CornerRadius = UDim.new(0, 6)
local vtPad = Instance.new("UIPadding")
vtPad.PaddingLeft = UDim.new(0, 8)
vtPad.PaddingRight = UDim.new(0, 8)
vtPad.Parent = vtBox
local vtGlow = Instance.new("UIStroke")
vtGlow.Color = Color3.fromRGB(120, 220, 255)
vtGlow.Thickness = 1.5
vtGlow.Transparency = 0.3
vtGlow.Parent = vtBox

local vtBtn = Instance.new("TextButton")
vtBtn.Size = UDim2.new(0.33, 0, 1, 0)
vtBtn.BackgroundColor3 = Color3.fromRGB(45, 45, 65)
vtBtn.Text = "Add"
vtBtn.Font = Enum.Font.GothamBold
vtBtn.TextSize = 10
vtBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
vtBtn.LayoutOrder = 2
vtBtn.Parent = vtRow
Instance.new("UICorner", vtBtn).CornerRadius = UDim.new(0, 6)
local vtStroke = Instance.new("UIStroke")
vtStroke.Color = Color3.fromRGB(120, 220, 255)
vtStroke.Thickness = 1.5
vtStroke.Transparency = 0.3
vtStroke.Parent = vtBtn

vtBtn.MouseButton1Click:Connect(function()
    local name = vtBox.Text
    if name == "" then
        game.StarterGui:SetCore("SendNotification", {Title = "Yuno Hub", Text = "Enter a pet name.", Duration = 3})
        return
    end
    local ok, msg = VisualAddToTrade(name, {
...
-- ============================================================
-- Theme Section (in the first tab, at the bottom)
-- ============================================================
local themeSecLabel = Instance.new("TextLabel")
themeSecLabel.Size = UDim2.new(1, 0, 0, 14)
themeSecLabel.BackgroundTransparency = 1
themeSecLabel.Text = "Theme"
themeSecLabel.Font = Enum.Font.GothamBold
themeSecLabel.TextSize = 11
themeSecLabel.TextColor3 = Color3.fromRGB(200, 180, 255)
themeSecLabel.TextXAlignment = Enum.TextXAlignment.Left
themeSecLabel.LayoutOrder = 11
themeSecLabel.Parent = petsPanel

local themeRow = Instance.new("Frame")
themeRow.Size = UDim2.new(1, 0, 0, 28)
themeRow.BackgroundTransparency = 1
themeRow.LayoutOrder = 12
...
themeRowLayout.Padding = UDim.new(0, 5)
...
-- Font selector
local fontSecLabel = Instance.new("TextLabel")
fontSecLabel.Size = UDim2.new(1, 0, 0, 14)
fontSecLabel.BackgroundTransparency = 1
fontSecLabel.Text = "Font"
fontSecLabel.Font = Enum.Font.GothamBold
fontSecLabel.TextSize = 11
fontSecLabel.TextColor3 = Color3.fromRGB(200, 180, 255)
fontSecLabel.TextXAlignment = Enum.TextXAlignment.Left
fontSecLabel.LayoutOrder = 13
fontSecLabel.Parent = petsPanel
...
fontDropdownBtn.LayoutOrder = 14
