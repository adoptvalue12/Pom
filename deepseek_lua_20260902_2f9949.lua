-- YUNO HUB ULTIMATE – STEAL AN EGG
-- Fusion de : Apostol Hub, StealAnEgg Public, NAZ GEN, AdamKRC
-- Interface : WindUI
-- Toutes les fonctionnalités : Auto Steal, Place, Hatch, Treadmill, Sell, Farm, ESP, PvP, Progression, Anti-Trap, Anti-Kick, Anti-AFK, Server Hop, Auto Collect, Auto Upgrade, Claim, Rebirth, Noclip, Speed Boost, Infinite Jump, et plus encore.
...
local function fireRemote(remote, ...)
    if remote then
        if remote:IsA("RemoteFunction") then return pcall(remote.InvokeServer, remote, ...)
        elseif remote:IsA("RemoteEvent") then return pcall(remote.FireServer, remote, ...) end
    end
    return false
end
...
function AntiDetect:Clean()
    pcall(function()
        for k in pairs(_G) do if type(k)=="string" and string.find(k,"Yuno") then _G[k]=nil end end
        ...
        if Y.AntiTrap then
            pcall(function()
                for _, obj in pairs(WS:GetDescendants()) do
                    ...
                    if string.find(n, "trap") or string.find(n, "mine") or string.find(n, "spike") or string.find(n, "laser") then
                        if obj:IsA("BasePart") then obj.CanCollide = false; obj.CanTouch = false
                        elseif obj:IsA("TouchInterest") then obj:Destroy() end
                    end
                end
            end)
        end
...
task.spawn(function()
    while true do
        ...
        local h = getHum()
        if h then
            if Y.SpeedBoost then
                h.WalkSpeed = Y.WalkSpeed
                h.JumpPower = Y.JumpPower
            else
                if h.WalkSpeed ~= 16 then h.WalkSpeed = 16; h.JumpPower = 50 end
            end
        end
    end
end)
            ...
            pcall(function()
                for _, plr in pairs(Players:GetPlayers()) do
                    if plr ~= LP and plr.Character and not playerHighlights[plr] then
                        ...
                        hl.FillColor = Color3.fromRGB(255, 0, 75)
            ...
            if not kickConn then
                ...
                if mt then
                    ...
                    mt.__namecall = function(self, ...)
                        if select(1, ...) == "Kick" and self == LP then return end
                        return old(self, ...)
                    end
                    kickConn = true
                end
...
function ServerHop.getServerList()
    local ok, result = pcall(function()
        return HS:JSONDecode(game:HttpGet(string.format("https://games.roblox.com/v1/games/%d/servers/Public?sortOrder=Asc&limit=100", game.PlaceId)))
    end)
...
function ServerHop.hopToEmpty()
    ...
    if id and count <= Y.MaxPlayers then
        task.wait(Y.HopDelay)
        Teleport:TeleportToPlaceInstance(game.PlaceId, id, LP)
    end
end
task.spawn(function()
    while true do
        if Y.ServerHop and #Players:GetPlayers() > Y.MaxPlayers then
...
local function stealEgg(egg)
    ...
    if prompt then
        ...
        Y.EggsStolen = Y.EggsStolen + 1
        return true
    end
    local remote = Remotes.Steal
    if remote then
        local ok = fireRemote(remote, "AreaEggs")
        if ok then
            Y.EggsStolen = Y.EggsStolen + 1
            return true
        end
...
local function placeEgg()
    local r = Remotes.Place
    if r then return fireRemote(r, true) end
    return false
end

local function hatchEgg()
    local r = Remotes.Hatch
    if r then
        fireRemote(r, true)
        task.wait(1)
        local f = Remotes.FinishHatch
        if f then fireRemote(f, true) end
        Y.EggsHatched = Y.EggsHatched + 1
        return true
    end
    return false
end
...
local function useTreadmill()
    local r = Remotes.Treadmill
    if r then return fireRemote(r, 100) end
    return false
end

local function sellEgg()
    local r = Remotes.Sell
    if r then return fireRemote(r, "Pet_123") end
    return false
end
...
task.spawn(function()
    while true do
        if Y.AutoSteal then
            ...
            task.wait(math.random(Y.StealDelayMin * 10, Y.StealDelayMax * 10) / 10)
        else
            task.wait(0.5)
        end
    end
...
task.spawn(function()
    while true do
        if Y.AutoFarm and not farmRunning then
            farmRunning = true
            pcall(function()
                if not Y.FarmBottomPos or not Y.FarmTopPos then
                    warn("Définis les positions BAS et HAUT")
                    farmRunning = false
                    return
                end
                local r = getHRP()
                local h = getHum()
                if not r or not h then farmRunning = false return end
                safeTeleport(Y.FarmBottomPos)
                task.wait(0.2)
                local egg = getClosestEgg()
                if egg then stealEgg(egg) end
                task.wait(0.3)
                local oldSpeed, oldJump = h.WalkSpeed, h.JumpPower
                h.WalkSpeed = Y.FarmSpeed or 250
                h.JumpPower = 100
                moveTo(Y.FarmTopPos)
                local timeout = 8
                local start = tick()
                while (r.Position - Y.FarmTopPos).Magnitude > 5 and tick() - start < timeout do
                    task.wait(0.05)
                end
                placeEgg()
...
local function findNearestEggCarrier()
    ...
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= LP and plr.Character then
            ...
            if r and playerHasEgg(plr) then
...
local function swingBat(target)
    ...
    local hr = RS:FindFirstChild("HitPlayer") or RS:FindFirstChild("SwingBat") or RS:FindFirstChild("Attack")
    if hr then
        fireRemote(hr, target.Character, Root.CFrame.LookVector * 50)
        return true
    end
    safeTeleport(tr.Position + Vector3.new(0, 0, -3))
    task.wait(0.1)
    return true
end
task.spawn(function()
    while true do
        if Y.PvPMode then
            pcall(function()
                for _, plr in pairs(Players:GetPlayers()) do
                    if plr ~= LP and plr.Character then
                        local has = playerHasEgg(plr)
                        if has and not pvpHighlights[plr] then
                            local box = Instance.new("SelectionBox")
                        ...
                        elseif not has and pvpHighlights[plr] then
                ...
                if target and dist <= Y.PvPRange then
                    swingBat(target)
                    task.wait(0.5)
                    local dropped = WS:FindFirstChild("DroppedEggs") or WS
                ...
                elseif target then
                    local tr = target.Character:FindFirstChild("HumanoidRootPart")
                    if tr then moveTo(tr.Position) end
                end
            end)
        end
        task.wait(0.2)
...
local TRAILS = {
    { name="Gray", price=100, mult=1.5 },
    { name="Green", price=5000, mult=2.0 },
    { name="Blue", price=75000, mult=2.5 },
    { name="Purple", price=1500000, mult=3.0 },
    { name="Golden", price=30000000, mult=3.5 },
    { name="Red", price=750000000, mult=4.0 },
    { name="Galaxy", price=20000000000, mult=5.0 },
    { name="Secret", price=500000000000, mult=7.0 },
    { name="Eternal", price=12500000000000, mult=10.0 },
    { name="Divine", price=13000000000000, mult=14.0 },
}
...
local function getBestAffordableTrail(money)
    local best = nil
    for _, t in ipairs(TRAILS) do
        if money >= t.price then best = t end
    end
    return best
end
local function upgradeGarden()
    local gardenBtn = WS:FindFirstChild("GardenUpgrade", true) or WS:FindFirstChild("UpgradeGarden", true)
    if gardenBtn then
        local p = gardenBtn.PrimaryPart or gardenBtn:FindFirstChildWhichIsA("BasePart")
        if p then
            safeTeleport(p.Position)
            local remote = RS:FindFirstChild("UpgradeGarden") or RS:FindFirstChild("BuyGarden") or RS:FindFirstChild("GardenUpgrade")
            if remote then fireRemote(remote) end
        end
    end
end
local function buyBestTrail()
    local money = getCurrentMoney()
    local trail = getBestAffordableTrail(money)
    if not trail then return end
    local trailShop = WS:FindFirstChild("TrailShop", true) or WS:FindFirstChild("Shop", true)
    if trailShop then
        local p = trailShop.PrimaryPart or trailShop:FindFirstChildWhichIsA("BasePart")
        if p then safeTeleport(p.Position) end
    end
    local remote = RS:FindFirstChild("BuyTrail") or RS:FindFirstChild("PurchaseTrail") or RS:FindFirstChild("Trail")
    if remote then fireRemote(remote, trail.name) end
end
local function fuseAllDuplicates()
    local beginFuse = GetRemote("RF/Fusery/BeginFuse")
    local confirmBrf = GetRemote("RF/Fusery/ConfirmBriefing")
    local finReveal = GetRemote("RF/Fusery/FinishReveal")
    if not beginFuse then return end
    local fusery = WS:FindFirstChild("Fusery", true) or WS:FindFirstChild("FuseMachine", true)
    if fusery then
        local p = fusery.PrimaryPart or fusery:FindFirstChildWhichIsA("BasePart")
        if p then safeTeleport(p.Position) end
    end
    for i = 1, 5 do
        pcall(function() beginFuse:InvokeServer() end)
        task.wait(0.3)
        if confirmBrf then pcall(function() confirmBrf:InvokeServer() end) end
        task.wait(0.3)
        if finReveal then pcall(function() finReveal:InvokeServer() end) end
        task.wait(0.5)
    end
end
local function upgradeTreadmill(times)
    times = times or 5
    local tierRaise = GetRemote("RF/Treadmill/AskTierRaise")
    if not tierRaise then return end
    local tm = WS:FindFirstChild("Treadmill", true)
    if tm then
        local p = tm.PrimaryPart or tm:FindFirstChildWhichIsA("BasePart")
        if p then safeTeleport(p.Position) end
    end
    for i = 1, times do
        pcall(function() tierRaise:InvokeServer() end)
        task.wait(0.3)
    end
end
local autoProgRunning = false
local autoProgStage = 0
task.spawn(function()
    while true do
        if Y.AutoProgression then
            if not autoProgRunning then
                autoProgRunning = true
                task.spawn(function()
                    local trainCycles = 0
                    while Y.AutoProgression do
                        local speed = getCurrentSpeed()
                        local money = getCurrentMoney()
                        local biome = getBestBiome(speed)
                        Y.TargetRarity = biome.rarity
                        Y.AutoSteal = true
                        Y.AutoHatch = true

                        if money >= 1000 and not WS:FindFirstChild("Treadmill", true) then
                            upgradeGarden()
                            task.wait(2)
                        end

                        trainCycles = trainCycles + 1
                        if trainCycles >= 10 then
                            trainCycles = 0
                            Y.AutoSteal = false
                            Y.AutoTreadmill = true
                            task.wait(30)
                            Y.AutoTreadmill = false
                            upgradeTreadmill(3)
                        end

                        local bestTrail = getBestAffordableTrail(money)
                        if bestTrail then
                            buyBestTrail()
                            task.wait(1)
                        end

                        if trainCycles % 5 == 0 then
                            fuseAllDuplicates()
                            task.wait(2)
                        end

                        Y.AutoSteal = true
                        autoProgStage = biome.stage or 1
                        task.wait(5)
                    end
                    autoProgRunning = false
                end)
            end
        else
            if autoProgRunning then
                autoProgRunning = false
                Y.AutoSteal = false
                Y.AutoTreadmill = false
            end
...
local function saveConfig()
    local data = {
        AutoSteal = Y.AutoSteal,
        AutoPlace = Y.AutoPlace,
        AutoHatch = Y.AutoHatch,
        AutoTreadmill = Y.AutoTreadmill,
        AutoSell = Y.AutoSell,
        AutoFarm = Y.AutoFarm,
        AntiTrap = Y.AntiTrap,
        Noclip = Y.Noclip,
        ESPEggs = Y.ESPEggs,
        ESPPlayers = Y.ESPPlayers,
        AntiKick = Y.AntiKick,
        AntiAFK = Y.AntiAFK,
        ServerHop = Y.ServerHop,
        AutoCollect = Y.AutoCollect,
        AutoUpgrade = Y.AutoUpgrade,
        AutoClaim = Y.AutoClaim,
        AutoRebirth = Y.AutoRebirth,
        PvPMode = Y.PvPMode,
        AutoProgression = Y.AutoProgression,
        SpeedBoost = Y.SpeedBoost,
        InfiniteJump = Y.InfiniteJump,
        MutationOnly = Y.MutationOnly,
        WalkSpeed = Y.WalkSpeed,
        JumpPower = Y.JumpPower,
        FarmSpeed = Y.FarmSpeed,
        StealDelayMin = Y.StealDelayMin,
        StealDelayMax = Y.StealDelayMax,
        TargetRarity = Y.TargetRarity,
        PvPRange = Y.PvPRange,
        MaxPlayers = Y.MaxPlayers,
        HopDelay = Y.HopDelay,
        FarmDelay = Y.FarmDelay,
        FarmBottomPos = Y.FarmBottomPos,
...
local function createUI()
    ...
    local function toggleCB(flag, label, default)
        Tab:Toggle({
            Flag = flag,
            Title = label,
            Default = Y[flag] ~= nil and Y[flag] or default,
            Callback = function(v)
                Y[flag] = v
                saveConfig()
            end
    ...
    Tab:Slider({
        ...
        Title = "Délai min (s)",
        Step = 0.1,
        Value = { Min = 0.2, Max = 3, Default = Y.StealDelayMin },
        Callback = function(v) Y.StealDelayMin = v; saveConfig() end
    })
    Tab:Slider({
        Flag = "StealDelayMax",
        Title = "Délai max (s)",
        Step = 0.1,
        Value = { Min = 0.5, Max = 5, Default = Y.StealDelayMax },
        Callback = function(v) Y.StealDelayMax = v; saveConfig() end
    })
    ...
    FarmTab:Slider({
        ...
        Step = 10,
        Value = { Min = 200, Max = 300, Default = Y.FarmSpeed or 250 },
        Callback = function(v) Y.FarmSpeed = v; saveConfig() end
    })
    FarmTab:Slider({
        Flag = "FarmDelay",
        Title = "Délai entre les cycles (s)",
        Step = 0.1,
        Value = { Min = 0.5, Max = 5, Default = Y.FarmDelay or 1.0 },
        Callback = function(v) Y.FarmDelay = v; saveConfig() end
    })
    ...
    PvPTab:Slider({
        ...
        Step = 1,
        Value = { Min = 5, Max = 100, Default = Y.PvPRange },
        Callback = function(v) Y.PvPRange = v; saveConfig() end
    })
    PvPTab:Button({
        Title = "🏏 Attaquer le joueur le plus proche",
        Icon = "sword",
        Callback = function()
            local target = findNearestEggCarrier()
            if target then
                swingBat(target)
    ...
    VisualsTab:Slider({
        ...
        Step = 1,
        Value = { Min = 16, Max = 200, Default = Y.WalkSpeed },
        Callback = function(v) Y.WalkSpeed = v; saveConfig() end
    })
    VisualsTab:Slider({
        Flag = "JumpPower",
        Title = "Puissance de saut",
        Step = 5,
        Value = { Min = 50, Max = 500, Default = Y.JumpPower },
        Callback = function(v) Y.JumpPower = v; saveConfig() end
    })
    ...
    ProtectionTab:Slider({
        ...
        Step = 1,
        Value = { Min = 2, Max = 10, Default = Y.MaxPlayers },
        Callback = function(v) Y.MaxPlayers = v; saveConfig() end
    })
    ProtectionTab:Slider({
        Flag = "HopDelay",
        Title = "Délai avant hop (s)",
        Step = 1,
        Value = { Min = 1, Max = 10, Default = Y.HopDelay },
        Callback = function(v) Y.HopDelay = v; saveConfig() end
    })
    ProtectionTab:Button({
        Title = "🔍 Chercher un serveur vide",
        Icon = "search",
        Callback = function() ServerHop.hopToEmpty() end
    })
    ...
    ProgressionTab:Label({
        Title = "💰 Argent : " .. getCurrentMoney(),
        Icon = "dollar-sign",
        Color = Color3.fromRGB(200, 200, 220)
    })

    -- Filtre rareté
    local RarityTab = Main:Tab({ Title = "Filtre", Icon = "filter" })
    RarityTab:Section({ Title = "Rareté des œufs", TextSize = 18 })
    local rarityOptions = {"Toutes", "Common", "Uncommon", "Rare", "Epic", "Legendary", "Mythic", "Divine"}
    RarityTab:Dropdown({
        Flag = "TargetRarity",
        Title = "Sélectionner la rareté cible",
        Options = rarityOptions,
        Default = Y.TargetRarity or "Toutes",
        Callback = function(v)
            Y.TargetRarity = v
            saveConfig()
        end
    })
    toggleCB("MutationOnly", "🔬 Uniquement les œufs Mutation", false)
    ...
    task.spawn(function()
        while true do
            local b = Y.FarmBottomPos and "✅" or "❌"
            local t = Y.FarmTopPos and "✅" or "❌"
            local s = Remotes.Steal and "✅" or "❌"
            local p = Remotes.Place and "✅" or "❌"
            statusLabel:Set({
                Title = string.format("Bas: %s | Haut: %s | Vol: %s | Pose: %s", b, t, s, p)
            })
            task.wait(2)
        end
    end)
    ...
    local statsLabel = StatsTab:Label({
        Title = string.format(
            "🥚 Œufs volés : %d\n🐣 Œufs éclos : %d\n⏱️ Temps : %d s",
            Y.EggsStolen, Y.EggsHatched, Y.SessionTime
        ),
        Icon = "bar-chart",
        Color = Color3.fromRGB(200, 200, 220)
    })
...
UIS.InputBegan:Connect(function(input, gp)
    ...
    if key == Enum.KeyCode.F1 then
        Y.AutoSteal = not Y.AutoSteal
        saveConfig()
        WindUI:Notify({ Title = "F1", Content = "Auto Steal : " .. (Y.AutoSteal and "ON" or "OFF"), Icon = "info", Duration = 2 })
    elseif key == Enum.KeyCode.F2 then
        Y.AutoFarm = not Y.AutoFarm
        saveConfig()
        WindUI:Notify({ Title = "F2", Content = "Auto Farm : " .. (Y.AutoFarm and "ON" or "OFF"), Icon = "info", Duration = 2 })
    elseif key == Enum.KeyCode.F3 then
        Y.SpeedBoost = not Y.SpeedBoost
        saveConfig()
        WindUI:Notify({ Title = "F3", Content = "Speed Boost : " .. (Y.SpeedBoost and "ON" or "OFF"), Icon = "info", Duration = 2 })
    elseif key == Enum.KeyCode.F4 then
        Y.Noclip = not Y.Noclip
        saveConfig()
        WindUI:Notify({ Title = "F4", Content = "Noclip : " .. (Y.Noclip and "ON" or "OFF"), Icon = "info", Duration = 2 })
    elseif key == Enum.KeyCode.F5 then
        Y.InfiniteJump = not Y.InfiniteJump
        saveConfig()
        WindUI:Notify({ Title = "F5", Content = "Infinite Jump : " .. (Y.InfiniteJump and "ON" or "OFF"), Icon = "info", Duration = 2 })
    elseif key == Enum.KeyCode.F6 then
        Y.AutoHatch = not Y.AutoHatch
        saveConfig()
        WindUI:Notify({ Title = "F6", Content = "Auto Hatch : " .. (Y.AutoHatch and "ON" or "OFF"), Icon = "info", Duration = 2 })
    elseif key == Enum.KeyCode.F7 then
        Y.AutoCollect = not Y.AutoCollect
        saveConfig()
        WindUI:Notify({ Title = "F7", Content = "Auto Collect : " .. (Y.AutoCollect and "ON" or "OFF"), Icon = "info", Duration = 2 })
    elseif key == Enum.KeyCode.F8 then
        Y.AutoRebirth = not Y.AutoRebirth
        saveConfig()
        WindUI:Notify({ Title = "F8", Content = "Auto Rebirth : " .. (Y.AutoRebirth and "ON" or "OFF"), Icon = "info", Duration = 2 })
    elseif key == Enum.KeyCode.F9 then
        Y.PvPMode = not Y.PvPMode
        saveConfig()
        WindUI:Notify({ Title = "F9", Content = "PvP Mode : " .. (Y.PvPMode and "ON" or "OFF"), Icon = "info", Duration = 2 })
    elseif key == Enum.KeyCode.F10 then
        Y.AutoProgression = not Y.AutoProgression
        saveConfig()
        WindUI:Notify({ Title = "F10", Content = "Auto Progression : " .. (Y.AutoProgression and "ON" or "OFF"), Icon = "info", Duration = 2 })
    end
end)
...
print("==========================================")
print("🥚 YUNO HUB ULTIMATE - STEAL AN EGG")
print("==========================================")
